...
WebIM.conn.listen({
	onOpened: function (message) { // 连接成功回调
		...
		const re =  '/contact' 
		const path = location.pathname.indexOf("login") !== -1 || location.pathname.indexOf("register") !== -1 ? re : location.pathname;
	...
	onTextMessage: function (message) {
		...
		Vue.$store.commit("updateMsgList", {
			...
			bySelf: false,
			ext: {
				goodsImg:message.ext.goodsImg || '',
				goodsName:message.ext.goodsName || '',
				goodsPrice:message.ext.goodsPrice || ''
			},
			from: message.from,
			mid: message.id
		});
	...
	onReceivedMessage: function (message) {
		console.log("onReceivedMessage1222", message);
